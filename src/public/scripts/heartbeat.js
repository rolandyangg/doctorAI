...
export class Heartbeat {
    ...
    try {
      this.stream = await navigator.mediaDevices.getUserMedia({
        video: {
          facingMode: 'user',
          width: {
            exact: this.webcamVideoElement.width
          },
          height: {
            exact: this.webcamVideoElement.height
          }
        },
        audio: false
      });
  ...
  async init() {
    ...
    try {
      ...
      const canvas = faceapi.createCanvasFromMedia(this.webcamVideoElement)
